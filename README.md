...
Streamlit 메인 페이지에 접속하면 배경과 함께 상단에 프로젝트 제목(SKN Animation)이 보이며, 가운데에는 임의로 재생되는 애니메이션 영상이 출력되어 애니메이션 분위기를 살립니다.
...
원하는 애니메이션의 제목이 정확히 기억나지 않더라도 일부 키워드(예: `one`)만 입력해도 해당 키워드가 포함된 애니메이션 리스트가 출력됩니다.  
해당 리스트에는 애니 이름, 장르, 평점이 함께 제공되며, 전체 제목을 쉽게 확인할 수 있어 검색 편의성이 높습니다.
...
원하는 장르를 자유롭게 입력하면, 해당 장르가 포함된 모든 애니메이션이 리스트로 출력됩니다.  
드라마, 학원, 스포츠 등 다양한 조합으로 필터링 가능하며, 이를 통해 내가 보고 싶은 스타일의 애니 제목을 손쉽게 찾을 수 있습니다.
...
애니메이션 제목을 클릭하면 해당 애니의 대표 이미지(포스터)가 자동으로 출력됩니다.  
하단의 `Watch Trailer` 버튼을 누르면, 다음 이미지(f)처럼 유튜브 트레일러 영상으로 바로 연결되어 영상으로 확인 가능합니다.
...
선택한 애니메이션의 트레일러 영상을 유튜브에서 바로 확인할 수 있어, 실제 영상 퀄리티나 분위기를 미리 파악할 수 있습니다.
...
애니메이션이 너무 재미있었다면, 그와 유사한 다른 애니를 보고 싶을 수 있습니다.  
이럴 때 `Recommendation Button`을 누르면, 내가 선택한 애니메이션과 유사한 내용을 가진 애니를 추천해줍니다.
...
검색한 애니메이션과 비슷한 장르와 내용을 가진 애니메이션 5개를 추천해줍니다.  
**직접 학습한 추천 모델(K-means, KNN, SVD, Hybrid 등)**을 기반으로 하여 사용자 취향에 맞는 결과를 Streamlit에서 실시간으로 제공합니다.
